		...
		try:
			...
			if method in ["set", "create", "add"] and len(arguments) >= 2:
				if messageRequest.price_alerts_available():
					...
					if payload is None or len(payload.get("candles", [])) == 0:
						...
						for alert in marketAlerts:
							...
							if currentAlertPlatform == currentPlatform and hash(dumps(alertTicker, option=OPT_SORT_KEYS)) == hashName:
								if alert["level"] == newAlert["level"]:
									embed = discord.Embed(title="Price alert for {} ({}) at {} {} already exists.".format(ticker.get("base"), currentAlertPlatform if not bool(exchange) else exchange.get("name"), levelText, ticker.get("quote")), color=constants.colors["gray"])
						...
						async with message.channel.typing():
							...
							embed = discord.Embed(title="Price alert set for {} ({}) at {} {}.".format(ticker.get("base"), currentPlatform if not bool(exchange) else exchange.get("name"), levelText, ticker.get("quote")), color=constants.colors["deep purple"])
				...
				for key, alert, matchedId in marketAlerts:
					...
					embed = discord.Embed(title="Price alert set for {} ({}) at {} {}".format(ticker.get("base"), currentPlatform if ticker.get("exchange") is None else ticker.get("exchange").get("name"), alert.get("levelText", alert["level"]), ticker.get("quote")), color=constants.colors["deep purple"])
